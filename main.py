...
def data_augmentation(images, gt_images):
    ...
    """
    flip_gen = np.random.randint(2, size=len(images))
    def flip_image(image, flag):
        if flag:
            return image[:, ::-1, :]
        return image    
    def augment_pipeline(image, flag):
        # random flip
        image = flip_image(image, flag)
        # random brightness
        image_hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
        random_bright = .25+np.random.uniform()
        image_hsv[:,:,2] = image_hsv[:,:,2]*random_bright
        return cv2.cvtColor(image_hsv,cv2.COLOR_HSV2RGB)
    augmented_images = [augment_pipeline(image, flag) for image, flag in zip(images, flip_gen)]
    augmented_gt_images = [flip_image(image, flag) for image, flag in zip(gt_images, flip_gen)]
...
def data_augmentation1(images, gt_images):
    ...
    # add guassian noises
    noises = np.random.normal(0, 0.055, shape)
