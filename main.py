...
def data_augmentation(images, gt_images, shape):
    """
    ...
    :param shape: the shape of an image
    ...
    random_gen = np.random.randint(2, size=(len(images),3))
    ...
    def blur_image(image, flag3):
        ...
        return image
    def augment_pipeline(image, flags, shape):
        """
        :flags: e.g [1,0,1]
        """
        # random flip
    ...
    augmented_gt_images = [gt_augment_pipeline(image, flags, shape) for image, flags in zip(gt_images, random_gen)]
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for batch_num, (X,y) in enumerate(get_batches_fn(batch_size)):
            if X.ndim == 4:
                X, y = data_augmentation(X,y,image_shape)
            feed_dict = {
                input_image: X,
                correct_label: y,
                keep_prob: 0.5,
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # Implemented in the 'data_augmentation' function
        ...
        train_nn(sess, num_epochs, batch_size, get_batches_fn, train_op, loss, input_image, correct_label,
                 keep_prob, learning_rate, image_shape)
